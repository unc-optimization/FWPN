function [y, lambda, obj_val, info] = LogRegFWPNSubSolver(x0, y0, A, tempt, C, muf, tol, max_iter)
...
obj_val  = 1/n*sum(log(temp)) + muf*sum((C*x0).^2)/2;

% solve the sub problem
d = temp1.*inv_temp;
[y, info] = QuadSimplex_away_logreg(grad - ((d.*Ax0)'*A)' - muf*CtC*x0, A, tempt, d, muf*CtC, y0, tol, max_iter);
temp = C*(y-x0);
AymAx0 = A*y - Ax0;
